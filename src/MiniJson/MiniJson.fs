...
module internal Details =
  ...
  let inline adv  (p : byref<int>       ) : unit = p <- p + 1
  ...
  let inline tryConsume_Delimiter (first : bool, v : IParseVisitor, s : string, pos : byref<int>) : bool =
    if first then true
    else
      tryConsume_Char         (',', v, s, &pos)
      && consume_WhiteSpace   (        s, &pos)

  let rec tryParse_ArrayValues (first : bool, sb : StringBuilder, v : IParseVisitor, s : string, pos : byref<int>) : bool =
  ...
  and tryParse_ObjectMembers (first : bool, sb : StringBuilder, v : IParseVisitor, s : string, pos : byref<int>) : bool =
    if test_Char ('}', s, pos) then
      true
    else
      tryConsume_Delimiter      (first,     v, s, &pos)
  ...
  let inline popContext
    (
      context             : Stack<BaseJsonValueBuilder>,
      freeArrayBuilders   : Stack<BaseJsonValueBuilder>,
      freeObjectBuilders  : Stack<BaseJsonValueBuilder>
    ) : Json =
    let v = context.Pop ()
